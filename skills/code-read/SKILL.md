...
└─ Phase C  未覆盖范围决策表生成            （主对话，解析 Phase B 产出的未覆盖项 + grep 联动预判，写 code-reads/<module>-coverage-decisions.md）
```
...
├─ Phase D.2  联动注解 subagent             （subagent 串行，每勾选项一次，产出 <new-module>-annotated.md + 联动块）
...
| 前置条件 | 无 | 无 | **必须已存在** `<module>-annotated.md` |
| 产出文件名 | `<module>.md` | `<module>-annotated.md` + `<module>-coverage-decisions.md` | `<new-module>-annotated.md` × N（N = 勾选项数）+ 原注解回填 |
...
| 变量 | 含义 | 解析方式 |
|------|------|---------|
| `SOURCE_ANNOTATED` | 原注解文件路径 | 用户贴的 annotated.md 路径，或从 `-coverage-decisions.md` 文件名推断（去 `-coverage-decisions` 后缀加 `-annotated`） |
| `DECISION_TABLE` | 决策表路径 | 用户贴的 `-coverage-decisions.md` 路径；若用户只贴 annotated.md 则推断 `code-reads/<module>-coverage-decisions.md`，不存在则跳过决策表读取 |
...
📋 决策表已生成：code-reads/<module>-coverage-decisions.md
...
Phase B 落盘 `<module>-annotated.md` 后，主对话**追加**写一份决策表到 `code-reads/<module>-coverage-decisions.md`。这是把"诚实声明"变成"可操作下一步"的桥梁——用户在表里勾选要深入哪些未覆盖项，下次调用 code-read 时贴这个决策表路径，skill 自动进联动续注模式。

### C.1 主对话职责

**不委托 subagent**——决策表生成是纯文本处理 + 一次轻量 grep，主对话自己做：

1. **解析未覆盖范围表**：读 `<module>-annotated.md` 末尾的「未覆盖范围」结构化表格（Phase B 已保证格式）
2. **联动预判 grep**：对每个未覆盖项 `item`，执行 `rg -n '<item>' <module>-annotated.md`，拿命中行号
...
4. **写决策表**到 `code-reads/<module>-coverage-decisions.md`
...
> 关联注解：code-reads/<module>-annotated.md
> 生成日期：<YYYY-MM-DD>
> 使用方法：
...
| # | 深入? | 未覆盖项 | 类型 | 位置/线索 | 未注解原因 | 建议优先级 | 联动预判（原注解引用点） |
|---|------|---------|------|----------|-----------|-----------|----------------------|
| 1 | - [ ] | UORManagerComponent | C++类 | Source/.../UORManagerComponent.h | C++侧未读 | 高 | Server:345 / Server:612 / Common:88 |
| 2 | - [ ] | MutableActorSubsystem | UE Subsystem | (UE Subsystem) | 未读 | 高 | Client:20 / Client:51 |
| 3 | - [ ] | G.Kittens.Future | Lua工具 | Common/.../G.lua | 未读 | 中 | Server:715 / Server:1045 |
| 4 | - [ ] | _GetROClass | 函数 | Server:711-736 | 异步加载委托AssetManager | 低 | （第四层索引） |
...
1. **决策表只是文本处理**——主对话**不读源码**，只 grep 已产出的 `<module>-annotated.md`
2. **联动预判列只列 grep 到的行号**——不推测、不脑补，0 命中就写"（无命中）"或"（第四层索引）"
3. **建议优先级判据公开透明**——按命中次数 + 所在层打分，不打"我觉得重要"这种主观分
4. **决策表与原注解同目录**——`code-reads/` 下，文件名 `-coverage-decisions` 后缀
...
写入 code-reads/<new-module-slug>-annotated.md，文件名沿用现有 ASCII 化规则
...
```markdown
原行内容
<!-- 联动 ↓ 详见 <new-module-slug>-annotated.md §X.Y（方向：原调用新 / 数据流：传入RO数据期望写入FastArray） -->
```
...
|---------|------|-------------|--------------|---------|------|
| UORManagerComponent | 原调用新 | Server:345 | uormanager-component-annotated.md §2.1 | 函数调用 | ✅ 已联动 |
| UORManagerComponent | 新回调原 | Server:612 | uormanager-component-annotated.md §2.3 | 回调 | ✅ 已联动 |
| UORManagerComponent | 数据共享 | Common:88 | uormanager-component-annotated.md §1 | 数据结构 | ✅ 已联动 |
| MutableActorSubsystem | 原调用新 | Client:20 | mutable-actor-subsystem-annotated.md §2.1 | 函数调用 | ✅ 已联动 |
| MutableActorSubsystem | 原调用新 | Client:51 | mutable-actor-subsystem-annotated.md §2.2 | 函数调用 | ✅ 已联动 |
...
```
✅ 已联动注解，详见 <new-module-slug>-annotated.md
```

若该行同时被多个新模块联动，列出所有新模块注解文件，用 `、` 分隔。
...
📄 新增注解：
  - <new-module-1>-annotated.md（<N1> 个函数含 🔗 联动块）
  - <new-module-2>-annotated.md（<N2> 个函数含 🔗 联动块）
...
需要继续深入其他未覆盖项吗？
（编辑 <module>-coverage-decisions.md 勾选新项后再次贴回本文件路径）
```

### Phase D 红线

1. **联动续注模式必须基于已存在的 `<module>-annotated.md`**——不存在则提示用户先跑注解模式建立基线
...
| 用户贴 `-coverage-decisions.md` 路径 | 联动续注模式（Phase D），读勾选项启动 D.1~D.4 |
| 用户说"继续深入 X、Y"未贴决策表 | 联动续注模式，X/Y 直接作为 SELECTED_ITEMS，跳过 D.1 的决策表读取，但仍 grep 原注解建 LINKAGE_INDEX |
